      ...
      <div className="container mx-auto px-4 py-6 md:py-10">
        ...
        <header className="text-center mb-6 md:mb-10">
          <div className="flex items-center justify-center gap-2 mb-3">
            <div
              className="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-xs font-bold text-white"
              aria-hidden="true"
            >
              TFF
            </div>
        ...
        {/* Question content */}
        <div className="max-w-2xl mx-auto py-6 md:py-8" role="form" aria-label="Quiz question">
          {renderQuestion()}
        </div>

        {/* Navigation buttons - only show during questions, not email capture */}
        {!isEmailCapture && (
          <nav className="max-w-2xl mx-auto mt-6" aria-label="Quiz navigation">
            <div className="flex justify-between items-center">
              ...
              <button
                ...
                className={cn(
                  ...
                  currentQuestion === 0
                    ...
                    : 'text-slate-600 hover:text-slate-800 hover:bg-slate-100 focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2'
                )}
              >
                <svg
                  className="w-5 h-5"
              ...
              <button
                ...
                disabled={!canAdvance}
                aria-label={currentQuestion === 7 ? 'Proceed to get your blueprint' : 'Continue to next question'}
                className={cn(
                  'flex items-center gap-2 px-6 py-3 rounded-xl font-semibold transition-all duration-200',
                  canAdvance
                    ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-lg hover:shadow-xl hover:from-indigo-700 hover:to-purple-700 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-indigo-600'
            ...
            {/* Keyboard hint */}
            {canAdvance && (
              <p className="text-center text-sm text-slate-400 mt-4" aria-hidden="true">
                Press <kbd className="px-2 py-0.5 bg-slate-100 rounded text-slate-600 text-xs">Enter</kbd> to continue
              </p>
            )}
          </nav>
        )}
